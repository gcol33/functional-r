...
Running totals: `1, 3, 6, 10, 15`. This is the *scan* operation (Haskell's `scanl`, APL's `\`). Where fold collapses a list to a single value, scan collapses it to a list of running values. Running maxima, cumulative sums, progressive string construction — all fall out of the same pattern:
...
Sometimes the operation at each step needs more than just the accumulator and the next element. `purrr::reduce2()` folds over two lists in parallel, passing an extra argument from the second list at each step:
