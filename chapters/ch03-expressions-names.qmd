...
Step back and notice what the workspace actually is: a collection of name-to-value bindings, exactly the mechanism this chapter introduced with `x <- 3`. The global environment is itself a value you can inspect (`environment()` returns it, `ls.str()` prints every binding in it), which means the thing that holds all your names is itself an expression that evaluates to a result. Even R's bookkeeping is expressions all the way down.
