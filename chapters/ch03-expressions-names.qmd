...
You just wrote a complete R program. No variables, no functions, no setup: it is an expression, and R evaluated it and printed the result. Every piece of R code you will ever write, from a one-line calculator to a thousand-line analysis pipeline, works exactly this way. You give R an expression; R gives you back its value.
...
The arrow `<-` is R's assignment operator. It binds the name `x` to the value `3`. After this line runs, typing `x` gives you back 3:
...
This "names are labels, not containers" model is a consequence of R's functional design. Values in R are immutable; when you "change" something, R creates a new value and moves the name to point at it. The old value stays untouched (and gets cleaned up automatically when nothing points to it anymore; that's the garbage collection R inherited from Lisp, @sec-lisp). If names were containers, you would need to worry about two containers sharing the same contents and one of them mutating behind your back. With labels and immutable values, that problem vanishes.
