...
Before a function can remember anything, you need to understand where R stores what it knows. An environment is R's internal bookkeeping structure for mapping names to values: think of it as a bag where each label points to exactly one object, and every bag has a parent bag above it, forming a chain that R walks whenever it needs to resolve a name. At the very top of this chain sits the empty environment, parentless; everything else inherits from something.
...
That accumulation is both a convenience and a trap. Convenient, because you can explore interactively, building data objects step by step. Dangerous, because any function defined in the global environment can see and silently depend on those objects: a function that works in your current session may break in a fresh one because it relied on a global variable you forgot to pass as an argument. In the language of lambda calculus, that function is an *open term*, its behavior hostage to whatever context it happens to run in. Making all dependencies explicit (passing them as arguments) turns it into a *closed term*, portable and self-contained. But what happens when a name *isn't* passed as an argument?

### The search path

When you type `x` at the console, R checks the global environment first; if no match turns up, it moves to the parent, then the parent's parent, walking up the chain until it either finds the name or runs out of environments. This chain is the *search path*:
...
The pattern is always the same: a factory function creates an environment, returns an inner function that captures it, and `<<-` lets the inner function modify the captured state. Because that state lives in a private environment that travels with the function, no outside code can see or tamper with it, and it persists across calls.

Now compare this to the global-variable approach:
...
These tools are not for production code (reaching into a closure's environment breaks its encapsulation), but for learning they are the fastest way to build intuition. The cycle of "make a closure, inspect its environment, call it, inspect again" turns an abstract concept into something you can watch happening, variable by variable, call by call. And once you have that intuition, you might start to wonder: if a closure can hold state, and a list of closures can impersonate an object, what exactly separates functional programming from object-oriented programming?
