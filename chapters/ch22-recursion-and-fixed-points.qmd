...
Start with the simplest case: the factorial. n! = n * (n-1) * ... * 1, with 0! = 1:
...
The accumulator pattern still buys you something. It clarifies the logic (the accumulator carries the "answer so far"), it is the prerequisite for trampolining (next section), and it turns exponential algorithms into linear ones:
...
R is not Scheme. It does not optimize tail calls, its stack is limited, and function call overhead is high compared to vectorized operations. Recursion in R is the right tool in specific situations and the wrong tool in many others. The shape of the data tells you which.

*Tree-shaped* problems — nested lists, file system traversal, parsing hierarchical data, divide-and-conquer algorithms where the depth is O(log n) — stay well within stack limits and express naturally as recursive functions.
