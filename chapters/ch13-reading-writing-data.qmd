...
It never converts strings to factors, it is faster on large files, it prints the column types it guessed so you can catch mistakes immediately, and it returns a tibble (@sec-tibbles) instead of a plain data frame. For most work, `read_csv()` is simply the better default.
...
::: callout-tip
## Opinion
Excel is a fine data source. It is a terrible data format. Merged cells, multiple tables crammed onto one sheet, values encoded by background color: none of these survive the trip to R intact. The safest workflow is to read from Excel, clean the data in R, then save as CSV or Parquet for everything downstream.
:::
