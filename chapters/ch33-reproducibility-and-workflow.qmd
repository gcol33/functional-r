...
The fix is structural, not behavioral. Do not rely on discipline ("I will remember to run script 3 first"). Build a system that makes the wrong thing hard. Each tool in this chapter eliminates a specific failure mode: `here` eliminates path problems, `renv` eliminates version problems, Quarto eliminates the code/output sync problem, `targets` eliminates the run-order problem, Git eliminates the "which version was it?" problem. The first one costs nothing to adopt.
...
Raw data is sacred: never modify it. Read it, transform it in code, write outputs elsewhere. If someone asks "where did this number come from?", you can trace it from the output file back through the code to the raw data. That traceability breaks the moment code and data live in different places.
...
Another common mistake: using `rm(list = ls())` at the top of a script to "clean up." This clears your workspace but does not restart R. Hidden state (loaded packages, modified options, changed working directories) persists. Instead, restart R (Ctrl+Shift+F10 in RStudio) for a true clean slate. Clean paths solve one reproducibility problem; package versions are the next one.
...
renv does not version R itself. If the R version matters, document it. Docker solves this completely (see @sec-reproducibility-stack), but is beyond this book. What renv does handle -- package versions -- raises an immediate practical question.

Should you commit `renv.lock` to Git? Yes, always. It is small (a few KB of JSON), and it is the entire point of renv. Without the lockfile in version control, a collaborator cannot restore your environment. The `renv/` directory itself (which contains the actual installed packages) is typically git-ignored.
...
Output formats: HTML, PDF, Word, presentations, websites, books. This book is written in Quarto. The format is flexible, but the real control comes from chunk options.
...
The citation is rendered in the output and a reference list is appended automatically. No more manually typed reference sections that drift from the actual citations. Quarto keeps code and prose in sync within a single document; for multi-step analyses, a different tool keeps the steps themselves in sync.
...
These are small things, but they close gaps that the larger tools leave open.
