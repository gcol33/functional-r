...
`x |> f()` is syntactic sugar for `f(x)`. R literally rewrites it before the code runs. Chain two and `x |> f() |> g()` becomes `g(f(x))`, which is (g ∘ f)(x) written left-to-right instead of inside-out. That is function composition from @sec-church, just in friendlier notation.
