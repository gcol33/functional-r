...
R sits at the opposite end of that spectrum: no static types, no compiler, no ahead-of-time guarantees. Every precondition you want enforced must be enforced by code you write yourself. That is a burden, but it has an unexpected upside. A dependently typed language lets you declare preconditions in type signatures, and the compiler checks them automatically. R gives you no such machinery; the only way to enforce a precondition is to write `if (bad) stop("why")` yourself. The question behind both approaches is the same — "what must hold for this computation to be sound?" — but R makes you spell out every assumption in code, which means every assumption is visible when you revisit the function six months later. A type signature encodes constraints concisely; an `if`/`stop()` block encodes them verbosely but legibly. The verbose version has its own value when debugging, because the precondition is right there in the function body, not hidden in a type annotation you have to decode.
