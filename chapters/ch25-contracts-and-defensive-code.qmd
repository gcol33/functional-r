The next question is what happens when your functions need to talk to the outside world, to files, APIs, databases, where errors are not bugs but facts of life.# Contracts and defensive code {#sec-contracts-and-defensive-code}
...
We opened with a silent failure: a function that returns the wrong answer without telling anyone. Every tool in this chapter, from `stop()` to `tryCatch()` to `match.arg()` to S7 typed properties, exists to prevent that silence. Validate at the boundary. Write error messages that say what was expected and what was received. Catch errors only when you have a recovery plan. And design interfaces that make misuse difficult in the first place, so the validation you do write is a safety net, not the only thing holding the bridge up.
