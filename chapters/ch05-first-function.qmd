...
You have been borrowing other people's work. `sqrt(9)`, `c(1, 2, 3)`, `sum(x > 70)`: every piece of R code you have written so far calls a function that someone else defined, tested, and packaged for you. At some point the problems you need to solve will stop fitting neatly into the functions that already exist. What does it take to write your own?
...
The second call works because `*` is vectorized (@sec-vectorized-ops). `double` doesn't need a loop to handle vectors; the body `x * 2` already works element by element. You get vectorization for free.
...
The body is a block of expressions. R evaluates them in order and returns the value of the last one. This is the same rule from @sec-expressions-all-the-way: a block in curly braces is an expression, and its value is the value of the last line.
...
This is the same positional-versus-named distinction from @sec-calling-functions, when we used `log(8, base = 2)`. The rule is the same everywhere because function calls all work the same way.
...
`...` collects all the arguments you pass, however many there are. `c(...)` combines them into a vector. This is a surprisingly deep idea. In Church's lambda calculus (@sec-church), a function takes exactly one argument. If you want two arguments, you write a function that returns another function:
...
Type `sqrt` without parentheses and R prints the function itself. A function is a value, the same kind of thing as a number or a string. You can assign it, put it in a list, or check its type:
...
`double` is a name bound to a value, and that value happens to be a function. `f <- double` gives the same function a second name. This is exactly what happened in @sec-names-binding when `y <- x` gave the number 3 a second name.
...
`function(x) x + 1` creates a function. The parentheses around it and `(10)` immediately call it with the argument 10. This is an *anonymous function*: a function with no name. It's the R equivalent of Church's lambda notation, `λx. x + 1` applied to 10 (@sec-church). You won't write anonymous functions often at this stage, but they become important when you pass functions to other functions in @sec-iteration-without-loops.

You can do anything with a function that you can do with a number: store it in a variable, pass it to another function, return it as a result. This is what McCarthy built into Lisp in 1958 (@sec-lisp), and R inherits it directly.
...
Yes. `scale` found `multiplier` in the workspace because it wasn't defined inside the function. R looks for names first in the function's local environment, then in the environment where the function was defined. This is lexical scoping, the idea R inherited from Scheme (@sec-scheme).
...
The practical rule: what happens inside a function stays inside the function. The full machinery behind this (environments, closures, and the scope chain) is in @sec-closures-and-scope.
...
This is a small habit with a large payoff. A function that fails immediately with a clear message is worth ten functions that silently produce garbage. @sec-contracts-and-defensive-code covers this in depth; for now, `stopifnot()` at the top of your functions is enough.
...
R's `function(n)` is `λn` with a different spelling. The reduction above is exactly what R does when you call `factorial(3)`: it substitutes 3 for `n`, evaluates the body, hits `factorial(2)`, substitutes again, and keeps going until it reaches the base case. Recursion is how Church's lambda calculus (@sec-church) handles repetition. There is no loop counter being incremented; the function just calls itself with a smaller argument until it stops.
...
R has a limit on how deep recursion can go (around 1000 calls by default). For most day-to-day R work, vectorized operations and functions like `map()` (@sec-iteration-without-loops) are better tools. But there is something strange about `factorial`: its base case and inductive step look exactly like a proof by mathematical induction. That resemblance is not a coincidence.
