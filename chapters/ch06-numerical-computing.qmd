...
How big is the gap? You can see the stored representation with `sprintf()`:
...
Individual floating-point traps lose a few digits. But when traps compound across a chain of operations, the entire result can become meaningless. The question is how to tell in advance.
...
The error can be many orders of magnitude larger than machine epsilon. The matrix is not singular and the code is correct --- the problem itself amplifies rounding errors. What, then, can an algorithm guarantee?
