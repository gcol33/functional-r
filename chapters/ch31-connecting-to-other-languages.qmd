...
One function call compiles, links, loads, and registers the function. The turnaround from edit to test is seconds. The convenience extends beyond compilation: Rcpp handles type conversion on both sides.
...
No `PROTECT`, no `UNPROTECT`, no `SEXP`. The type conversion between R and Rust is handled by extendr's `#[extendr]` macro (used in source files) or inferred by `rust_function()`. The question is whether the safety gains justify the ecosystem trade-off.
...
Set this early, before calling `import()`. Once the Python interpreter starts, you cannot switch to a different one within the same R session. Get this wrong, and you will spend an hour debugging import errors that have nothing to do with your code.
