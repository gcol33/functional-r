...
A mathematical notation from 1936, no computers, no hardware, nothing but pencil and paper. A statistical programming language used by millions in 2025. Between those two points: four stops, each of which left a permanent mark on R. An AI lab at MIT in the late 1950s, a pair of graduate students at the same lab in the 1970s, a statistics department at Bell Labs, and a classroom in Auckland.
...
`my_function` is a value. You could put it in a list, pass it to another function, or replace it. That idea came from Lisp. But Lisp itself had a problem.
...
Lisp and Scheme were built by computer scientists for computer scientists. Neither had anything to say about data.


## S makes it practical {#sec-s-language}

That changed at Bell Labs.
...
S was eventually commercialized as S-PLUS, sold first by StatSci, then by Insightful Corporation, and finally by TIBCO, which acquired Insightful for $25 million in 2008. By then, S had a free competitor that had already overtaken it. It started with a corridor conversation in New Zealand.
...
R looks almost identical to S on the surface, but under the hood it is semantically closer to Scheme. The question is which pieces came from where.
...
Five lines, four ancestors. The rest of this book is about what you can do with these features once you understand them. The most fundamental one: in R, everything is an expression, and every expression has a value.
