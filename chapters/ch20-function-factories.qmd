...
Formatting, filtering, statistical transformations — any task where you repeat the same logic with different parameters is a factory waiting to happen.
...
The `if` runs once, when the factory is called, not every time the produced function is called. The factory picks the right formula at creation time and returns a function that does no branching. The same pattern drives much of ggplot2. Functions like `scale_color_brewer()` and `theme_minimal()` are factories or factory-like: they take parameters and return objects that modify a plot.
...
`safely(log)` is a function operator applied to `log`. The result is passed to `map()`, a functional from @sec-passing-functions. Three concepts from three chapters, working together. What if you need both safety *and* logging on the same function?
