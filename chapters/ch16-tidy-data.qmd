...
Now you can filter by year, group by country, plot year on the x-axis. Every dplyr verb and every ggplot2 aesthetic mapping works because each variable has its own column. The tools compose because the data has a consistent shape — but getting there by hand is tedious.
...
`pivot_wider()` is the inverse: values in a column become column names. You need it when data is *too* long — when distinct variables are stacked into a single column.
...
The workflow is: receive data however it comes, reshape to tidy for analysis, reshape to wide for presentation if needed. Tidy data is a means to easier analysis, not a goal in itself.

Even tidy data hits a wall when the information you need spans multiple tables.


## Joins: combining tables {#sec-joins}

You have customers and orders, species and observations, sites and measurements. Joins match rows from two tables by a shared key.
...
Pivoting and joining handle the structural problems. A few more tidyr functions handle the messy details.
