...
Under the hood, `filter()` is doing the same logical subsetting from @sec-comparison-operators: `penguins[penguins$species == "Adelie", ]`. The verb makes the intent explicit — and, more importantly, composable with other verbs.
...
`arrange()` is pure display. It does not change the data, only the order of rows. No information is added or removed. But a well-sorted table can reveal patterns that a jumbled one hides — and sometimes you need more than the five core verbs to expose them.
...
`across()` applies a function to multiple columns at once. It pairs naturally with `where()`, `starts_with()`, and the other selection helpers from @sec-select. The same pattern — "apply a function to many things" — reappears in a broader context in @sec-iteration-without-loops.
