...
Capturing *your own* expression and capturing *the caller's* expression require different tools.
...
This three-step sequence — **capture**, optionally **modify**, then **evaluate** in the right context — is what tidy evaluation calls defuse-and-inject. The next question is what happens when the expression you need does not exist yet.
...
What if the function name or variable is not known until runtime? You need to build the expression from parts.
