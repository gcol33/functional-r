...
`str()` is the most useful function for inspecting lists. It prints the structure compactly, showing types, lengths, and nesting levels. Use it whenever you receive an unfamiliar object. Every nested list is a tree: the top-level list is the root, each element is a branch, and atomic values are leaves. The same structure appears in file systems (directories containing files and subdirectories), HTML (the DOM), and JSON.

Inspecting is half the problem; the other half is reaching into the tree to pull out what you need. Chain `[[` or `$`:
...
JSON from a web API, the output of `lm()`, and configuration files are all nested lists or objects built on them. The access pattern is always the same: `$` or `[[` to step down one level, repeated as many times as needed.
...
Data frames (@sec-data-frames), model objects, and environments are all lists (or list-like structures) with different rules about what they can contain. The most important rule is also the simplest: what happens when every element has the same length?
