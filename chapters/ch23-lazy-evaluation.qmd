...
The mechanism behind all of this is promises. Once you see how they work, non-standard evaluation, data masking, and `{{ }}` follow as consequences. Full metaprogramming is in @sec-metaprogramming; here, the goal is understanding the mechanism well enough to use it.
...
But `subset(df, x > 3)` cannot work that way. If R evaluated `x > 3` in your environment, it would fail: there is no `x`. Instead, `subset` captures the *expression* before evaluating it, then decides where to evaluate it. Consider what happens:
