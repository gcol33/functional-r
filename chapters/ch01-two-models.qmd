...
Alonzo Church\index{Church, Alonzo}, who happened to be Turing's doctoral advisor at Princeton, was working on the same problem at the same time but from a completely different angle. Church built a notation called the *lambda calculus*\index{lambda calculus}. It has three things:

1. Variables: `x`
2. Functions: `λx. x + 1` (take `x`, return `x + 1`)
3. Application: `(λx. x + 1)(5)` gives `6`

There is no tape, no memory, no state that changes over time. You write an expression\index{expression} and simplify it by applying functions to arguments until you reach a value that can't be simplified further. Church proved that this system, despite having no concept of memory or instructions, can compute exactly the same class of functions as Turing's machine. The equivalence, known as the Church-Turing thesis\index{Church-Turing thesis}, meant that memory and instructions were never necessary for computation in the first place. Everything a Turing machine can do, pure expression can do without a single mutable cell.
