...
The same shape runs through the rest of the language. In R, `if/else` is an expression: it produces a value, so you can assign its result directly (@sec-expressions-names).

```r
y <- if (x > 0) "positive" else "negative"
```

Functions are ordinary values. You can store one in a variable, pass it to another function, or create one on the fly without ever giving it a name (@sec-functions-are-values). Operations on data work the same way: `x * 2` multiplies every element of `x` at once, with no loop and no index variable; the expression takes the whole vector as input, not one piece at a time (@sec-vectors). Pipe chains compose functions left to right, each one receiving what the last produced; this is exactly what Church meant by composition, with a syntax wrapped around it (@sec-pipes-and-composition).

```r
data |> filter(x > 5) |> summarize(mean(y))
```

All four come from a single decision: build the language out of expressions, and let functions be values like anything else. The shape is Mr. Fold's.
