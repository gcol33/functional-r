...
`nchar()` counts characters. `paste()` joins with a space, `paste0()` joins without. `sprintf()` does formatted substitution, borrowing syntax from C. These work, but they are inconsistent in argument order and `NA` handling — and inconsistency compounds once you start chaining operations.
...
Pattern matching is where stringr earns its place. `str_detect()` is the readable version of `grepl()`:
...
So when do factors actually help?
...
The `levels` argument controls the allowed values and their order. Without it, R defaults to alphabetical, which is why "high" comes before "low" and plots look wrong. Manually specifying levels for every factor gets tedious fast.
...
R returns a `difftime` object. Subtraction works, and so does addition — but only with scalars:
...
The `format` argument uses `%Y` (4-digit year), `%m` (month), `%d` (day), and similar codes. These are hard to remember — and easy to get wrong when your data mixes formats.
