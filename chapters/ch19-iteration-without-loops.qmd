...
Functions are values (@sec-functions-are-values-chapter) and you can pass them to other functions (@sec-passing-functions). A *functional* takes that one step further: it is a higher-order function that handles the iteration for you. You give it a function and a collection; it applies the function to each element, manages the indexing and pre-allocation, and returns the results.
...
That last point deserves a second look. A `for` loop iterates by maintaining a counter variable: `i` goes from 1 to `n`, and the body runs once per increment. You declare the counter, increment it, read it. With `map()`, the counter disappears. The list has three elements, so the function runs three times; the structure itself determines how many applications occur. Church's lambda calculus pushes the idea further still: the number 3 is `λf. λx. f(f(f(x)))`, a function that applies another function three times. The number does not *describe* iteration; it *is* iteration. `map()` sits between the explicit counter and the Church numeral, which is part of why it feels like a natural step away from loops. You stop managing the iteration and let the data's shape manage it for you.

But type stability is only half the story. The typed variants enforce something even stricter.
...
Under the hood, `across()` iterates over the selected columns, applies the function to each one, and assembles the results. That is the same thing `map()` does over list entries and `sapply()` does over vector values. The `for` loop at the start of this chapter, `lapply()`, `map_dbl()`, `across()`: each separates *what to do* from *how to iterate*, and each walks a different container with the same logic. The loop at the start of this chapter and the `across()` at the end look nothing alike, but they are the same act: applying a function to each element of a collection. In a loop, you write the iteration. In a functional, the language handles it.
